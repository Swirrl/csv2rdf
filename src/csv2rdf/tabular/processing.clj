...
(defn validate-merge-table [validating? dialect {:keys [url] :as user-table} table-resolver]
  (let [table-source (table-resolver url)
        embedded-metadata (csv/extract-embedded-metadata table-source dialect)
...
(defn ^{:table-spec "6.1"} get-metadata
  ([tabular-source metadata-source] (get-metadata tabular-source metadata-source identity))
  ([tabular-source metadata-source table-resolver]
   (cond
     (some? metadata-source)
     (let [{:keys [dialect tables] :as user-table-group} (meta/parse-table-group-from-source metadata-source)
           validating? false
           merged-tables (mapv (fn [table] (validate-merge-table validating? dialect table table-resolver)) tables)
